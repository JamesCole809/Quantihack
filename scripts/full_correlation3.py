...
climate = pd.read_csv('data/archive21/train_timeseries/train_timeseries.csv',
    usecols=['date','PRECTOT','T2M','T2M_MAX','T2M_MIN','WS10M','PS','T2MDEW'],
    nrows=5000000)
...
a23 = solar.groupby('Year').agg(
    ...
    solar_avg_ghi=('GHI', 'mean'), solar_avg_dni=('DNI', 'mean'),
    solar_avg_dhi=('DHI', 'mean'), solar_avg_wind=('Wind Speed', 'mean'),
    solar_avg_humidity=('Relative Humidity', 'mean'), solar_avg_pressure=('Pressure', 'mean'),
).reset_index()
...
print("\n" + "=" * 130)
print("TOP 50 CROSS-ARCHIVE CORRELATIONS")
print("=" * 130)
...
# ---- Top with 5+ years ----
print("\n" + "=" * 130)
print("TOP CORRELATIONS WITH 5+ OVERLAPPING YEARS")
print("=" * 130)
long_results = [r for r in results if r[5] >= 5]
